...
if (!menuShown()) {
  ?>
  <a href="#" class="btn btn-warning goPro" style="float:right" data-toggle="tooltip" title="Get the Pro Version Now for $15.95!"> <i class="glyphicon glyphicon-shopping-cart"></i> Buy Plugin Check Pro Now!</a>
  <?php
}
?>
<h2>Plugin Check <br>
...
?>
<p><em>Plugin Check</em> is a smart and useful developer tool, and a quality checker for the end user. It scans the applications and plugins you specify for undefined functions and methods.</p>

<h4>Features</h4>

<ol>
  <li>Modern Admin Interface: <em>Plugin Check</em> sports a modern and beautiful admin interface based on the twitter bootstrap framework.</li>
  <li>Admin Interface Tour: A slick tour will take you around the admin page and familiarize you with its features.</li>
  <li>Generous Help: Whenever you need help, the information and hint is only a click away in <em>Plugin Check</em>. (In fact, it is only a mouseover away.)</li>
  <li>Robust Security: Unbreakable authentication (using hash and salt), impervious to SQL injection etc.</li>
  <li>WordPress Integration: Plugin Check comes with built-in WordPress integration. It works as a WordPress plugin if uploaded to the <code>wp-content/plugins</code> folder of your blog. What's more, you can switch to the standalone mode from the WordPress plugin admin page of this application, while still using the WordPress authentication mechanism and database.</li>
</ol>

<p><em>Plugin Check</em> is available as a freely distributed <a href="http://buy.thulasidas.com/lite/plugin-check-lite.zip" title="Get Plugin Check Lite">lite version</a> and a <a href="http://buy.thulasidas.com/plugin-check" title="Get Plugin Check Pro for $5.95" class="goPro">Pro version</a>, which adds a couple of extra features.</p>
...
<ol>
  ...
  <li>Plugin Check can handle only one level of class inheritance.</li>
</ol>